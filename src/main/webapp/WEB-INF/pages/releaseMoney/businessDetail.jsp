	...
	<div data-options="region:'north'" title=""
		...
		<div style="width: 90%; margin-top:7px;margin-left:20px">
			...
			<label style="margin-left: 20px">记账日:</label>
				<input id="startKeepAccountsDate" type="text" class="easyui-datebox" style="width:100px;margin-top:50px;"
					data-options="formatter:myformatter,parser:myparser" /> 至
			...
			<a style="margin-left:18px;margin-top:3px;height:23px" id="button-search" href="javascript:;"	onclick="query()" class="easyui-linkbutton"	data-options="iconCls:'icon-search'">搜索</a>
			<a style="margin-left:30px;margin-top:3px;height:23px" id="button-search" href="javascript:;"	onclick="clearForm('queryForm')" class="easyui-linkbutton"	data-options="iconCls:'icon-reload'">重置</a>
		</div>
		</form>
	</div>
				...
				<tr>
					<td>
						...
						<div id="tb" style="margin-left: 20px">
						    ...
						    <a href="${CONTEXT_PATH}/businessLoanServer/excelExport.do" class="easyui-linkbutton" data-options="iconCls:'icon-save',plain:'true'">数据导出</a>
		...
		<table id="clearanceCertificateGrid"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'paySum'" width="70px">支付金额</th>
...
<script type="text/javascript">
	...
	function accountingMarkSubmit() {
		$('#accountingMarkForm').form('submit', {
			url : contextPath + '/businessLoanServer/accountingMark.do',
			success : function(data) {
				closeWin('accountingMarkdlg');
				var jsondata = eval("(" + data + ")");
				//是否有数据
				if (data == "[]") {
					$.messager.alert('提示', '暂无匹配数据!账务标记修改失败!', 'warning');
					//清空表单数据
					$('#clearanceCertificateGrid').datagrid('loadData', {
						total : 0,
						rows : []
