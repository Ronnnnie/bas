...
<!-- 数据网格 -->
	<div class="easyui-layout" data-options="fit:true" style="width:100%;height:98%">
		<div data-options="region:'north'" title="" style="width:100%;height: 80px;"> 
			<form id="queryForm">
		...
		<div id="tb" style="margin-left：100px">	
		   		...
		   		<tr>
		   			<td>
					    ...
					    <div id="tb" style="margin-left:20px">
							...
							<a href="http://localhost:8080/bas/clientRefundServer/exportExcel.do" class="easyui-linkbutton" data-options="iconCls:'icon-save',plain:true">模板导出</a>
		  ...
		  <table id="clearanceCertificateGrid" class="easyui-datagrid" title="放款表资金明细" style="width:100%;height:100%;"
			data-options="
					id:'dg-dopts',
					rownumbers:true,
					loadMsg:'数据加载中,请稍等',
		...
		<table id="orgInfoAddTable" style="margin:auto">
			<tbody>
				...
				<tr style="line-left:100px;">
					<th style="text-align: right; font-size: 13"><samp
							style="color: red">*</samp>请选择收款日期:</th>
					<td><input class="easyui-datebox" type="text"
...
<script type="text/javascript">
	...
	function updateData() {
		$('#orgInfoEditForm').form('submit', {
			url : contextPath + '/clientRefundServer/updateBusinessDetail.do',
			success : function(data) {
				//关闭窗口
	...
	function payConfirmSubmit() {
		$('#payConfirmForm').form('submit', {
			url : contextPath + '/clientRefundServer/payConfirm.do',
			success : function(data) {
	...
	function deleteData() {
		$.messager.confirm('操作提示','您确认要删除该用户？',function(r){    
		    if (r){ 
				$('#orgInfoEditForm').form('submit', {
					url : contextPath + '/clientRefundServer/deleteBusinessDetail.do',
					success : function(data) {
						//关闭窗口
						closeWin('ediTdlg');
			...
			if (fileName == "") {
				...
				if (d1 == ".xls" || d1 == ".xlsx") {
					...
					$("#uploadCntractExcelFrm").form("submit", {
						url : contextPath + '/clientRefundServer/importContract.do',
						onSubmit : function() {
						},
						success : function(result) {
							...
							if(obj.success){
								...
								$.messager.confirm('操作提示','请仔细核对数据无误后点击确定再次导入!',function(r){   
								    if (r){
								    	$.ajax({  
								    		url : contextPath + '/clientRefundServer/importContractSave.do',
								    	    type: "POST",  
								    	    success: function (data) {  
